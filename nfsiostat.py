#!/usr/bin/python
# -*- python-mode -*-
# V0.1 Added ability to send output of the main stats to graphite via the -g and -k options
# modified by Joshua Jackson <tsunam@gmail.com>
...
class DeviceData:
    ...
    def __graphite_rpc_op_stats(self, op, sample_time):
        """Print generic stats for one RPC op
        """
        if op not in self.__rpc_data:
            return
        ...
        export = self.__nfs_data['mountpoint']"/", "_")
        prefix = 'servers.%s.nfsiostat.%s.%s' % (hostname, export, rw)
        graphdata = ([])
        graphdata.append(( prefix + ".ops", ( seconds, ops / sample_time)))
        graphdata.append(( prefix + ".kb", ( seconds, kb_per_op)))
        graphdata.append(( prefix + ".retrans", ( seconds, retrans)))
        graphdata.append(( prefix + ".rtt", ( seconds, rtt_per_op)))
        graphdata.append(( prefix + ".exe", ( seconds, exe_per_op)))
        return graphdata
    ...
    def graphite_send(self, message, graphite, port):
        s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        s.connect((graphite, port))
        try:
          s.sendall(message)
        except socket.error, (value,message):
          print("count not send message:", message)
        s.close()

    def display_iostats(self, sample_time, *opts):
        ...
        if graphite:
          if which == 0:
              read=self.__graphite_rpc_op_stats('READ', sample_time)
              write=self.__graphite_rpc_op_stats('WRITE', sample_time)
              combined=read + write
          elif which == 1:
              getattr=self.__graphite_rpc_op_stats('GETATTR', sample_time)
              access=self.__graphite_rpc_op_stats('ACCESS', sample_time)
              attr=self.__graphite_attr_cache_stats(sample_time)
              combined=getattr + access + attr
          elif which == 2:
              lookup=self.__graphite_rpc_op_stats('LOOKUP', sample_time)
              readdir=self.__graphite_rpc_op_stats('READDIR', sample_time)
              if 'READDIRPLUS' in self.__rpc_data:
                  readdirplus=self.__graphite_rpc_op_stats('READDIRPLUS', sample_time)
              cache=self.__graphite_dir_cache_stats(sample_time)
              combined=lookup + readdir + readdirplus + cache
          elif which == 3:
              read=self.__graphite_rpc_op_stats('READ', sample_time)
              write=self.__graphite_rpc_op_stats('WRITE', sample_time)
              combined=read + write
          payload = pickle.dumps(combined)
          header = struct.pack("!L", len(payload))
          message = header + payload
          self.graphite_send(message, graphite, port)
        else:
          if which == 0:
...
def print_iostat_summary(old, new, devices, time, options):
    ...
    for device in devicelist:
        if old:
            ...
            stats[device].display_iostats(time, options.which, options.graphite, options.pickle)
...
"""
    ...
    graphitegroup = OptionGroup(parser, "Graphite Options", "Options for feeding data to Graphite")
    graphitegroup.add_option('-g', '--graphite', dest="graphite", help="Graphite host address")
    graphitegroup.add_option('-k', '--pickle', dest="pickle", help="Graphite Pickle Port")
    parser.add_option_group(graphitegroup)
